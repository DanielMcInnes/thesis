...
procedure findquotedstring_packet is new findquotedstring(
   ...
   begin
      ...
      GetVariableStatusEnumType.FromString(NonSparkTypes.packet.To_String(dummybounded), self.attributeStatus, valid);
      if (valid = false) then NonSparkTypes.put_line("Invalid [object Object]"); return; end if;
      ...
      self.attributeValue := NonSparkTypes.GetVariableResultType.strattributeValue.To_Bounded_String(NonSparkTypes.packet.To_String(dummybounded));
 
      ocpp.findQuotedKeyQuotedValue(msg, msgIndex, valid, "component", dummybounded);
      if (valid = false) then NonSparkTypes.put_line("Invalid [object Object]"); return; end if;

      ComponentType.parse(dummybounded,msgindex, self.component, valid);
   ...
   begin
      ...
      retval := NonSparkTypes.packet.To_Bounded_String(""
                                                      ...
                                                      & "    " & '"' & "attributeValue" & '"' & ": " & '"' & NonSparkTypes.GetVariableResultType.strattributeValue.To_String(Self.attributeValue) & '"' & "," & ASCII.LF
